...
RSpec.describe OmniExchange do
  ...
  context 'the .get_fx_data' do
    ...
    it 'returns a hash with the converted amount, an exchange rate, and the data provider' do
      ...
      expect(response).to be_a(Hash).and contain_exactly([:converted_amount, converted_amount], [:exchange_rate, 0.9521811e1], [:fx_rate, 0.9521811e1], [:provider, :open_exchange_rates])
    end
